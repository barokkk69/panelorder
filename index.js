...
//▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰//
// [FIX] List semua Server (Owner/Reseller) - Tanpa Limit
//▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰//

bot.command("listsrv", async (ctx) => {
    const userId = ctx.from.id;

    // 1. Pengecekan Otorisasi (Sudah benar)
    if (!isOwner(userId)) {
    ...
    try {
        await ctx.reply("⏳ Mengambil daftar server, mohon tunggu...");

        // 2. Ambil SEMUA server (per_page=10000)
        let serverRes = await fetch(`${domain}/api/application/servers?per_page=10000`, {
            method: 'GET',
            headers: {
                'Accept': 'application/json',
                'Authorization': `Bearer ${plta}`
        ...
        // 3. [PERBAIKAN] Loop melalui 'servers' (bukan 'serversToShow')
        // Ini akan menampilkan SEMUA server
        for (let server of servers) {
            const s = server.attributes;
            let runtimeStatus = "Status T/D"; 
            
            try {
                // Ambil status real-time
                ...
                console.error(`Gagal ambil status untuk ${s.identifier}:`, error.message);
            }
        ...
        // 4. [PERBAIKAN] Hapus 'if' yang menampilkan "...server lainnya."
        // Sekarang semua server sudah ditampilkan di atas.

        messageText += `Total Server Ditemukan: ${servers.length}`;
