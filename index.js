...
bot.on("callback_query", async (ctx) => {
  try {
    ...
    // [START] LOGIKA PEMBAYARAN ORDER KUOTA
    if (cmd === "ORDER") {
      const username = arg2;
      const products = readProducts();
      ...
      // Cek apakah user masih punya order pending (opsional, untuk mencegah spam unik)
      const pendingOrder = Object.values(orders).find(o => o.buyer === ctx.from.id && o.status === "pending");
      if (pendingOrder) {
          return ctx.answerCbQuery("Selesaikan transaksi sebelumnya atau batalkan dulu!", { show_alert: true });
      }

      // --- KONFIGURASI ORDER KUOTA ---
      // Pastikan data ini ada di file config/setting Anda
      const orkutConfig = {
          apikey: config.ORKUT_APIKEY,   // Masukkan API Key Orkut
          username: config.ORKUT_USERNAME, // Masukkan Username Orkut
          token: config.ORKUT_TOKEN      // Masukkan Token/PIN Transaksi Orkut
      };
      
      const APIGATEWAY_ORKUT = "https://orkut-sand.vercel.app/orderkuota"; 

      // 1. Hitung Nominal Unik
      // Harga + Kode Unik (1 - 150 perak) agar mudah terdeteksi di mutasi
      const fee = Math.floor(Math.random() * 150) + 1; 
      const amount = parseInt(product.price);
      const totalBayar = amount + fee;
      const reff = `ORD-${Date.now()}`; // ID Referensi Lokal

      await ctx.reply(`🔄 <b>Memproses Tagihan...</b>\nMohon tunggu sebentar, sedang meminta QRIS.`, { parse_mode: 'HTML' });

      try {
        // 2. Request QRIS ke API
        const resOrkut = await axios.get(`${APIGATEWAY_ORKUT}/createpayment`, {
            params: {
                apikey: orkutConfig.apikey,
                amount: totalBayar,
                username: orkutConfig.username,
                token: orkutConfig.token // Terkadang butuh token/pin
            }
        });

        if (!resOrkut.data.status || !resOrkut.data.result) {
             throw new Error("Gagal mengambil QRIS dari Order Kuota.");
        }

        const qrImageUrl = resOrkut.data.result.imageqris.url || resOrkut.data.result.imageqris;

        // 3. Kirim Invoice QRIS ke User
        const sentMsg = await ctx.replyWithPhoto({ url: qrImageUrl }, {
          caption: `🧾 <b>INVOICE PEMBAYARAN</b>

📦 <b>Produk :</b> Panel ${product.name}
💰 <b>Harga Dasar :</b> Rp${new Intl.NumberFormat('id-ID').format(amount)}
🔢 <b>Kode Unik :</b> Rp${fee}
──────────────────────
💵 <b>TOTAL BAYAR : Rp${new Intl.NumberFormat('id-ID').format(totalBayar)}</b>
──────────────────────
⚠️ <b>PENTING:</b>
1. Wajib transfer sesuai <b>TOTAL BAYAR</b> (sampai digit terakhir).
2. QRIS Expired dalam 5 menit.
3. Sistem akan memproses otomatis setelah pembayaran diterima.`,
          parse_mode: 'HTML',
          ...Markup.inlineKeyboard([
            [Markup.button.callback("❌ Batalkan Pesanan", `CANCEL|${reff}`)]
          ])
        });

        // 4. Simpan order ke orders.json
        orders[reff] = {
            reff, 
            ...
            status: "pending",
            created: Date.now(),
            totalBayar: totalBayar, // PENTING: Disimpan untuk pengecekan mutasi
            qrisMessageId: sentMsg.message_id,
            chatId: ctx.chat.id
        };
        saveOrders(orders);

        // 5. POLLING CHECK MUTASI (Looping)
        let attempts = 0;
        const interval = setInterval(async () => {
            attempts++;
            
            // Batas waktu 5 menit (20 x 15 detik)
            if (attempts > 20) { 
                clearInterval(interval);
                const currentOrder = orders[reff];
                
                if (currentOrder?.status === "pending") {
                    // Hapus Invoice
                    try { await bot.telegram.deleteMessage(currentOrder.chatId, currentOrder.qrisMessageId); } catch (e) {}
                    
                    delete orders[reff];
                    saveOrders(orders);
                    await ctx.reply(`⏳ <b>Waktu Habis!</b>\nSilakan buat pesanan ulang jika ingin melanjutkan.`, {parse_mode: 'HTML'});
                }
                return;
            }

            // Jika order sudah dihapus/dibatalkan manual oleh user
            if (!orders[reff]) {
                clearInterval(interval);
                return;
            }
            ...
            try {
                // 6. Cek Mutasi di Order Kuota
                const resMutasi = await axios.get(`${APIGATEWAY_ORKUT}/mutasiqr`, {
                    params: {
                        apikey: orkutConfig.apikey,
                        username: orkutConfig.username,
                        token: orkutConfig.token
                    }
                });

                const mutasiList = resMutasi.data?.result;

                let isPaid = false;
                if (Array.isArray(mutasiList)) {
                    // Cek apakah ada nominal kredit yang SAMA PERSIS dengan totalBayar
                    isPaid = mutasiList.some(trx => {
                        // Ambil angka saja dari string (misal "Rp 10.501" jadi 10501)
                        const kreditStr = trx.kredit || trx.amount || "0"; 
                        const kredit = parseInt(kreditStr.toString().replace(/[^0-9]/g, ""));
                        return kredit === totalBayar;
                    });
                }
                ...
                if (isPaid) {
                    ...
                    // Hapus pesan QRIS agar bersih
                    try { await bot.telegram.deleteMessage(ctx.chat.id, sentMsg.message_id); } catch(e) {}

                    await ctx.reply(`✅ <b>PEMBAYARAN DITERIMA!</b>\nNominal Rp${totalBayar} telah masuk.\nSedang membuat panel Anda...`, {parse_mode: 'HTML'});
                    
                    // 7. EKSEKUSI PEMBUATAN PANEL (Logic asli Anda)
                    const account = await createUserAndServer({ username, product });
                    
                    if (account.error) {
                        await ctx.reply("⚠️ Pembayaran sukses tapi gagal membuat panel otomatis.\nHarap hubungi admin dengan screenshot bukti bayar.");
                        // Kirim log error ke admin
                        await bot.telegram.sendMessage(config.ADMIN_ID, `ERROR CREATE PANEL:\nUser: ${username}\nErr: ${JSON.stringify(account.details)}`);
                        return;
                    }

                    // 8. Kirim Detail Akun ke User
                    await bot.telegram.sendMessage(ctx.from.id, `┌  <b>PANEL BERHASIL DIBUAT</b> ┐

Berikut adalah data login Anda.
Mohon simpan baik-baik.

╭─ 🔐 <b>AKUN ANDA</b> ─────────
│ 👤 <b>Username:</b> <code>${account.username}</code>
│ 🔑 <b>Password:</b> <code>${account.password}</code>
...
╭─ ⚙️ <b>SPESIFIKASI</b> ───────
│ ⚡ <b>RAM:</b> ${product.ram === "0" ? "Unlimited" : product.ram + " MB"}
│ 💾 <b>DISK:</b> ${product.disk} MB
│ 🧠 <b>CPU:</b> ${product.cpu}%
╰─────────────────────
...
• RAM : ${product.ram} MB`, { parse_mode: 'HTML' }).catch(err => {
    console.log("Gagal kirim notif ke channel:", err.message);
                ...
                console.error("Error Checking Mutasi Orkut:", e.message);
                // Jangan break loop, coba lagi di interval berikutnya
            }

        }, 15000); // Cek setiap 15 detik

      } catch (e) {
        console.error("Orkut Create Payment Error:", e);
        return ctx.reply("❌ Gagal membuat tagihan QRIS. Sistem sedang sibuk.");
      }
